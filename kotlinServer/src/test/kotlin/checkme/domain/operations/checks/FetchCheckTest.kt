...
class FetchCheckTest : FunSpec({
    ...
    val fetchCheckByIdMock: (UUID) -> Check? = { id -> checks.firstOrNull { it.id == id } }
    val fetchChecksByUserIdMock: (UUID) -> List<Check>? = { id -> checks.filter { it.userId == id } }
    val fetchChecksByUserIdNullMock: (UUID) -> List<Check>? = { null }
    val fetchChecksByTaskIdMock: (UUID) -> List<Check>? = { id -> checks.filter { it.taskId == id } }
    val fetchChecksByTaskIdNullMock: (UUID) -> List<Check>? = { null }
    ...
    test("Fetch check by id should return an error if id is not valid") {
        fetchCheckById(notExistingId).shouldBeFailure(CheckFetchingError.NO_SUCH_CHECK)
    }
    ...
    test("Fetch check by user id should return an error if user id is not valid") {
        fetchChecksByUserIdNull(notExistingId).shouldBeFailure(CheckFetchingError.NO_SUCH_CHECK)
    }
    ...
    test("Fetch check by task id should return return an error if check id is not valid") {
        fetchChecksByTaskIdNull(notExistingId).shouldBeFailure(CheckFetchingError.NO_SUCH_CHECK)
    }
